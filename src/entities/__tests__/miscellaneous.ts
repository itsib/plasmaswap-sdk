import { ChainId, Token, TokenAmount, Pair, InsufficientInputAmountError, LiquidityProvider, sortedInsert } from '../../index';
...
describe('miscellaneous', () => {
  it('getLiquidityMinted:0', async () => {
    ...
    const liquidity = pair.getLiquidityMinted(new TokenAmount(pair.liquidityToken, '0'), new TokenAmount(tokenA, '1001'), new TokenAmount(tokenB, '1001'));
  ...
  it('getLiquidityMinted:!0', async () => {
    ...
    expect(pair.getLiquidityMinted(new TokenAmount(pair.liquidityToken, '10000'), new TokenAmount(tokenA, '2000'), new TokenAmount(tokenB, '2000')).raw.toString()).toEqual('2000');
  });
  ...
  it('getLiquidityValue:!feeOn', async () => {
    ...
    // 500
    {
      const liquidityValue = pair.getLiquidityValue(tokenA, new TokenAmount(pair.liquidityToken, '1000'), new TokenAmount(pair.liquidityToken, '500'), false);
      expect(liquidityValue.token.equals(tokenA)).toBe(true);
    ...
    // tokenB
    {
      const liquidityValue = pair.getLiquidityValue(tokenB, new TokenAmount(pair.liquidityToken, '1000'), new TokenAmount(pair.liquidityToken, '1000'), false);
