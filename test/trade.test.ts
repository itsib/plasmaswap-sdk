...
describe('Trade', () => {
  ...
  it('can be constructed with ETHER as input for exact output', () => {
    const trade = new Trade(new Route([pair_weth_0], ETHER, token0), new TokenAmount(token0, JSBI.BigInt(100)), TradeType.EXACT_OUTPUT);
    expect(trade.inputAmount.currency).toEqual(ETHER);
    expect(trade.outputAmount.currency).toEqual(token0);
  });

  it('can be constructed with ETHER as output', () => {
    const trade = new Trade(new Route([pair_weth_0], token0, ETHER), CurrencyAmount.ether(JSBI.BigInt(100)), TradeType.EXACT_OUTPUT);
    expect(trade.inputAmount.currency).toEqual(token0);
    expect(trade.outputAmount.currency).toEqual(ETHER);
  });
  it('can be constructed with ETHER as output for exact input', () => {
    const trade = new Trade(new Route([pair_weth_0], token0, ETHER), new TokenAmount(token0, JSBI.BigInt(100)), TradeType.EXACT_INPUT);
    expect(trade.inputAmount.currency).toEqual(token0);
    expect(trade.outputAmount.currency).toEqual(ETHER);
  });
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency output', () => {
      const result = Trade.bestTradeExactIn([pair_weth_0, pair_0_1, pair_0_3, pair_1_3], new TokenAmount(token3, JSBI.BigInt(100)), ETHER);
      expect(result).toHaveLength(2);
